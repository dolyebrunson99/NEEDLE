...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        other_size = prod(tuple(s if s != -1 else 1 for s in new_shape))
        new_shape = tuple(s if s != -1 else size//other_size for s in new_shape)
    ...
    def broadcast_to(self, new_shape):
        ...
        assert len(new_shape) == self.ndim, f"New shape{new_shape} must agree with array dimension {self.ndim}"
        new_strides = list(self.strides)
        for i in range(self.ndim):
            if new_shape[i] != self.shape[i]:
                assert self.shape[i] == 1,\
                       f"Can not broadcast dimension {i} from {self.shape[i]} to {new_shape[i]}"
                new_strides[i] = 0
    ...
    def __getitem__(self, idxs):
        ...
        for i, idx in enumerate(idxs):
            new_shape[i] = round((idx.stop - idx.start) / idx.step)
            new_stride[i] = self.strides[i] * idx.step
            offset += self.strides[i] * idx.start
        return NDArray.make(new_shape, tuple(new_stride), self.device, self._handle, offset)
    ...
    def flip(self, axes):
        ...
        if axes is None:
            ...
            axes = (axes,) # the axes has to be in tuple form

        offset = 0
        expanded_strides = (self.size,) + self.strides
        for axis in axes:
            offset += expanded_strides[axis] - expanded_strides[axis+1]
        strides = tuple(-s if i in axes
                           else s 
                           for i, s in enumerate(self.strides))
        return self.as_strided(self.shape, strides=strides, offset=offset).compact()
    ...
    def pad(self, axes):
        ...
        new_shape = tuple(shape+pad_left+pad_right for (pad_left, pad_right), shape in zip(axes, self.shape))
        out = self.device.full(shape=new_shape,
                               fill_value=0.)
        idx = tuple(slice(pad_left, 
                          -pad_right if pad_right else None, 
                          None) for pad_left, pad_right in axes)
        out[idx] = self
        return out
