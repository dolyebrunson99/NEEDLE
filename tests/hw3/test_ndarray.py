...
@pytest.mark.parametrize("keep_dims", keepdim_params)
def test_reduce_sum(params, keep_dims, device):
    dims, axis = params["dims"], params["axis"]
    _A = np.random.randn(*dims)
    A = nd.array(_A, device=device)
    np.testing.assert_allclose(
        _A.sum(axis=axis, keepdims=keep_dims), A.sum(axis=axis, keepdims=keep_dims).numpy(), atol=1e-5, rtol=1e-5
    )
...
@pytest.mark.parametrize("params", reduce_params)
@pytest.mark.parametrize("keep_dims", keepdim_params)
def test_reduce_max(params, keep_dims, device):
    dims, axis = params["dims"], params["axis"]
    _A = np.random.randn(*dims)
    A = nd.array(_A, device=device)
    np.testing.assert_allclose(
        _A.max(axis=axis, keepdims=keep_dims), A.max(axis=axis,  keepdims=keep_dims).numpy(), atol=1e-5, rtol=1e-5
    )
